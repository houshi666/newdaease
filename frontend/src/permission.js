...
const whiteList = ['/login', '/sso', '/401', '/404', '/nolic', '/de-auto-login', '/delink']; // no redirect whitelist
...
router.beforeEach(async (to, from, next) => routeBefore(() => {
  ...
  if (hasToken) {
    console.log("permisson.js--85---to.path:", to.path)
    if (to.path === '/login') {
      ...
      if (hasGetUserInfo || to.path.indexOf('/previewScreenShot/') > -1 || to.path.indexOf('/preview/') > -1 || to.path.indexOf('/nolic') > -1) {
        next()
        console.log("permisson.js--97")
        store.dispatch('permission/setCurrentPath', to.path)
      } else {
        if (store.getters.roles.length === 0) { // 判断当前用户是否已拉取完user_info信息
          console.log("permisson.js--101")
          // get user info
          store.dispatch('user/getInfo').then(() => {
            console.log("permisson.js--104")
            const deWebsocket = getSocket()
            deWebsocket && deWebsocket.reconnect && deWebsocket.reconnect()
            store.dispatch('lic/getLicInfo').then(() => {
              console.log("permisson.js--108")
              loadMenus(next, to)
            }).catch(() => {
              loadMenus(next, to)
            })
            ...
            store.dispatch('user/logout').then(() => {
              console.log("permisson.js--115")
              location.reload() // 为了重新实例化vue-router对象 避免bug
            })
          })
        } else if (store.getters.loadMenus) {
          console.log("permisson.js--117")
          // 修改成false，防止死循环
          store.dispatch('user/updateLoadMenus')
          store.dispatch('lic/getLicInfo').then(() => {
            console.log("permisson.js--124")
            loadMenus(next, to)
          }).catch(() => {
            console.log("permisson.js--127")
            loadMenus(next, to)
          })
        } else {
          next()
    ...
    if (whiteList.indexOf(to.path) !== -1) {
      console.log("permisson.js--134")
      // in the free login whitelist, go directly
      next()
    } else {
      // other pages that do not have permission to access are redirected to the login page.
      // next(`/login?redirect=${to.fullPath}`)
      console.log("permisson.js--140")
      console.log("permisson.js--147---to.fullPath---->", to.fullPath)
      // let encodeFullPath = encodeURIComponent(to.fullPath)
      let encodeFullPath = to.fullPath
      console.log("permisson.js--149---encodeURI--fullPath---->", encodeFullPath)
      if (encodeFullPath && (encodeFullPath.indexOf('delink') > -1 || JSON.stringify(to.path).indexOf('link') > -1)) {
        console.log("permisson.js--151----看板页面：>", encodeFullPath)
        window.location.href = window.location.origin + `/de-api/api/auth/sso/authorize?redirect=${encodeFullPath}`;
      } else {
        window.location.href = window.location.origin + '/de-api/api/auth/sso/authorize';
      }
      NProgress.done()
    }
  }
...
export const loadMenus = (next, to) => {
  console.log("loadMenus的next参数----->", JSON.stringify(next))
  console.log("loadMenus的to参数----->", JSON.stringify(to))
  buildMenus().then(res => {
    const data = res.data
    ...
    debugger
    if (to.query.visitToken) {
      console.log("跳转看板路径170----->11: ")
      if (JSON.stringify(to.fullPath).indexOf('delink') > -1 && JSON.stringify(to.query).indexOf('visitToken') > -1) {
        console.log("跳转看板路径172----->22: ")
        asyncRouter.push({
          path: '/delink',
          query: {link: to.query.link, user: to.query.user},
        })
      }
    }

    if (to.query.visitCode) {
      console.log("跳转看板路径180----->22: ")
      if (JSON.stringify(to.fullPath).indexOf('delink') > -1 && JSON.stringify(to.query).indexOf('visitCode') > -1) {
        console.log("跳转看板路径182----->33: ")
        asyncRouter.push({
          path: '/delink',
          query: {link: to.query.link, user: to.query.user},
        })
