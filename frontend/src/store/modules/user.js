import { login, logout, deLogout, getInfo, getUIinfo, languageApi, ssoLogin} from '@/api/user'
...
const actions = {
  ...
  ssoLogin({ commit }, reqObj) {
    return new Promise((resolve, reject) => {
      ssoLogin(reqObj.code, reqObj.redirect).then(response => {
        const { data } = response
        commit('SET_TOKEN', data.token)
        commit('SET_LOGIN_MSG', null)
        setToken(data.token)
