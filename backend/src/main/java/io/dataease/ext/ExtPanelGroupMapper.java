...
public interface ExtPanelGroupMapper {
    ...
    int insertNative(PanelGroupWithBLOBs var1);

    List<PanelGroupExtDto> getPanelListByTenantId(@Param("tenantId") String tenantId);

    List<PanelGroupExtDto> getPanelById(@Param("panelId") String panelId);

    int updatePictureUrl(@Param("pictureUrl") String pictureUrl, @Param("resourceId") String resourceId);
}
