...
public class PanelGroupService {
    ...
    public String save(PanelGroupRequest request) {
        ...
        request.setTenantId(AuthUtils.getUser().getUsername());
        extPanelGroupMapper.insertNative(request);
        // 清理权限缓存
    ...
    /**
     * @param tenantId 租户id
     * @description: 根据租户id查询看板列表
     * @author: youlei
     * @date: 2023/5/18 14:05
     * @return: java.util.List<io.dataease.dto.panel.PanelGroupExtDto>
     */
    public List<PanelGroupExtShowDto> getPanelListByTenantId(String tenantId) {
        List<PanelGroupExtShowDto> resultList = new ArrayList<>();

        List<PanelGroupExtDto> list = extPanelGroupMapper.getPanelListByTenantId(tenantId);
        for (PanelGroupExtDto extDto : list) {
            PanelGroupExtShowDto showDto = new PanelGroupExtShowDto();
            showDto.setId(extDto.getId());
            showDto.setTenantId(extDto.getTenantId());

            PanelLinkMappingExample example = new PanelLinkMappingExample();
            example.createCriteria().andResourceIdEqualTo(extDto.getId());
            ...
            if (mappings != null && mappings.size() > 0) {
                ...
                String webPath = pictureWebPath + contextPath + SHORT_URL_PREFIX + (StringUtils.isBlank(uuid) ? mapping.getId() : uuid);
                showDto.setWebUrl(webPath);
                showDto.setPictureUrl(extDto.getPictureUrl());
