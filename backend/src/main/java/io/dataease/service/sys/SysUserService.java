...
public class SysUserService {
    ...
    public List<SysUserGridResponse> queryForTenant(KeyGridRequest request) {
        String keyWord = request.getKeyWord();
        GridExample gridExample = request.convertExample();
        gridExample.setExtendCondition(AuthUtils.getUser().getUsername());
        List<SysUserGridResponse> lists = extSysUserMapper.queryForTenant(gridExample);
        lists.forEach(item -> {
            List<SysUserRole> roles = item.getRoles();
            List<Long> roleIds = roles.stream().filter(ObjectUtils::isNotEmpty).map(SysUserRole::getRoleId).collect(Collectors.toList());
            item.setRoleIds(roleIds);
...
//        checkEmail(request);
    ...
    public void ssoAuthorize() {
        ...
        url.append(ssoAuthConfig.getOauthServerUri());
        String req = UriComponentsBuilder.fromHttpUrl(url.toString())
                .queryParam("response_type", "code")
                .queryParam("client_id", ssoAuthConfig.getClientId())
                .queryParam("redirect_uri", ssoAuthConfig.getRedirectUri())
                .build().encode().toUriString();
        ResponseEntity<String> response = restTemplate.exchange(req, HttpMethod.GET, new HttpEntity<>(new LinkedMultiValueMap<>()), String.class, new HashMap<>(16));
    ...
    public String loginCode(String code) {
        ...
        ResponseEntity<String> response = restTemplate.exchange(queryUrl, HttpMethod.GET, new HttpEntity<>(new LinkedMultiValueMap<>()), String.class, new HashMap<>(16));
        JSONObject tokenInfo = JSONUtil.parseObj(response.getBody());
        if (tokenInfo.getInt("code") == 200) {
            JSONObject json = tokenInfo.getJSONObject("data");
            return portalUserInfo(json.getStr("access_token"));
        }
        return null;
    }
     ...
     * @param token
     */
    public String portalUserInfo(String token) {
        StringBuffer url = new StringBuffer(16);
        ...
        if (userInfo.getInt("code") == 200) {
            ...
            this.authUserService.getUserByName(userName);
            // 执行登录操作
            return "token";
        } else {
